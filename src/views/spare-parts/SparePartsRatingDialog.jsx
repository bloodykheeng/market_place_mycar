...
const SparePartsRatingDialog = ({
  ...
  const useCreateMutation = useMutation({
    mutationFn: postSparePartReview,
    onSuccess: () => {
  ...
  const useEditMutation = useMutation({
    mutationFn: (variables) =>
      updateSparePartReview(initialData?.id, variables),
    onSuccess: () => {
  ...
  const handleSubmit = () => {
    console.log("🚀 ~ SparePartsRatingDialog ~ pendingData:", pendingData);

    if (!initialData) {
      let finalData = {
        ...pendingData,
        spare_part_id: productDetailItem?.id
      };
...
export default SparePartsRatingDialog;
