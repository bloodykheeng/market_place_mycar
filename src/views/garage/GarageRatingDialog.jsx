...
const GarageRatingDialog = ({
  ...
  const useEditMutation = useMutation({
    mutationFn: (variables) => updateGarageReview(initialData?.id, variables),
    onSuccess: () => {
  ...
  const handleSubmit = () => {
    ...
    if (!initialData) {
      let finalData = {
        ...pendingData,
        garage_id: productDetailItem?.id
      };
...
export default GarageRatingDialog;
