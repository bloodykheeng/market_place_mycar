...
const CarInspectorsRatingDialog = ({
  onClose,
  show,
  initialData,
  loggedInUserData,
  ...
  const useEditMutation = useMutation({
    mutationFn: (variables) => updateUserReview(initialData?.id, variables),
    onSuccess: () => {
  ...
  const handleSubmit = () => {
    console.log("🚀 ~ CarInspectorsRatingDialog ~ pendingData:", pendingData);

    if (!initialData) {
      let finalData = {
        ...pendingData,
        user_id: productDetailItem?.id
      };
...
export default CarInspectorsRatingDialog;
